...
int do_miss(int c_addr){
	...
	if(Cache[cache_index].tag != c_tag){
		while(Cache[cache_index].sca == 1 && cache_index<127){
			Cache[cache_index].sca = 0;
			cache_index++;
		}
		if(cache_index > 127)
			cache_index = 0;
		...
		//write-back
		for(int i=0; i<16; i++){
			int store_addr = (Cache[cache_index].tag << 6) | (i*4);
			if(Cache[cache_index].data[i] != 0){
				Memory[store_addr/4] = Cache[cache_index].data[i];
			}
			Cache[cache_index].data[i] = 0;
		}
	}	

	Cache[cache_index].valid = 1;
        Cache[cache_index].tag = c_tag;
	return 0;
}	

int ReadMem(int addr){
	//divide address
	int current_tag = addr >> 6;
	int offset = addr & 0x3f;
	
	//dirty
	int dirty_index = 0;
	if(cache_index != dirty_index) Cache[cache_index].dirty = 0;

	//Cache Hit
	for(int j=0; j<128; j++){
		if((Cache[j].valid == 1) && (Cache[j].tag == current_tag) && Cache[j].dirty != 1){
			hit++;
                	//printf("\tHIT!(%x,%x,%x)\n",j,current_tag,offset);

			if(Cache[j].data[offset/4] == 0)
				Cache[j].data[offset/4] = Memory[addr/4];
			Cache[j].sca = 1;
			return Cache[j].data[offset/4];
		}
	}
	...
	if(Cache[cache_index].tag != current_tag)
		dirty_index = cache_index;

	Cache[cache_index].data[offset/4] = Memory[addr/4];

	return Cache[cache_index].data[offset/4];
}
...
void WriteMem(int addr, int value){
	...
	Cache[cache_index].data[offset/4] = value;
